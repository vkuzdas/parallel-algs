...
void writeRecords(const vector<vector<int>> &records, string filePath) {
    // opens file on filepath, as output, as binary, trunc == overwrite
    ofstream bout(filePath.c_str(), ofstream::out | ofstream::binary | ofstream::trunc);

    // how many records will follow
...
//void readBins(string file) {
//    int content = 0;
//    string prefix = "../../src/public/solutions/";
//    string path = prefix + file;
//
//    ifstream bin(path.c_str(), ifstream::in | ofstream::binary);
//    bin.read((char*)&content, sizeof(int));
//
//    printf("f: %s  c: %d\n", file.c_str(), content);
//}
